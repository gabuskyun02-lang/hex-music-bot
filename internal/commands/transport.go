...
func Stop(b *hexbot.Bot, event *events.ApplicationCommandInteractionCreate, _ discord.SlashCommandInteractionData) error {
	...
	if player == nil || player.Track == nil {
		return b.ReplyEmbed(event, hexbot.ErrorEmbed("Nothing is playing — queue cleared"))
	}
	return b.ReplyEmbed(event, hexbot.SuccessEmbed("⏹ Stopped and cleared the queue"))
}
...
func Join(b *hexbot.Bot, event *events.ApplicationCommandInteractionCreate, _ discord.SlashCommandInteractionData) error {
	...
	return b.ReplyEmbed(event, hexbot.SuccessEmbed("Joined <#"+voiceState.ChannelID.String()+">"))
}
...
func Leave(b *hexbot.Bot, event *events.ApplicationCommandInteractionCreate, _ discord.SlashCommandInteractionData) error {
	...
	if !b.IsDJ(event) {
		return b.ReplyEmbed(event, hexbot.ErrorEmbed("You need the DJ role to disconnect the bot"))
	}

	player := b.Lavalink.ExistingPlayer(guildID)
	if player == nil {
		return b.ReplyEmbed(event, hexbot.ErrorEmbed("I am not connected to voice"))
	}
	...
	return b.ReplyEmbed(event, hexbot.SuccessEmbed("👋 Left the voice channel"))
}
